...
legend {
  ...
  input[type=text]{
    width:100%;
    height:30px;
    border: 2px solid rgb(221, 143, 40);
    border-radius:5px;"
  }
...
<form method="post" action="updateSales.php">
  ...
  <input type="date" name="date" style="width:100%;height:30px;
   border: 2px solid rgb(221, 143, 40);border-radius:5px;" required>
  <br><br>
  <input type="number" name="total" pattern = "[0-9]+" min="0" style="width:100%;height:30px;
  ...
  border: 2px solid rgba(249, 105, 14, 1); cursor:pointer;background-color: rgb(221, 143, 40);">&ensp; 
</fieldset> 
...
if ($con->connect_error) {
  ...
  $exe = mysqli_query($con,$qry2);
  $count = mysqli_fetch_row($exe);
  if($count[0]==1)
  {
